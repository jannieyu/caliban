class Brush {
  ...
  set size(newSize) {
    ...
    if (newSize > 0 &&
        newSize < this._height / 2 &&
        newSize < this._width / 2 &&
        newSize !== this._size) {
  ...
  draw(ctxDst, sx, sy, swidth, sheight, mag) {
    ...
    ctxDst.drawImage(
      ...
      ctxDst.canvas.width - 2 * this._padding,
      ctxDst.canvas.height - 2 * this._padding
    );
    ctxDst.restore();
    ...
    if (this.show) {
      ...
      ctxDst.strokeRect(
        (this.threshX - sx) * mag + this._padding, // boxStartX
        (this.threshY - sy) * mag + this._padding, // boxStartY
        (this.x - this.threshX) * mag, // boxWidth
        (this.y - this.threshY) * mag // boxHeight
      );
    }
  }
}
